/**
 * ProductSingle 展示层 init 配置（init_script_url，与 html_url 对称）
 */
...
export interface ProductSingleClientInitConfig extends ComponentsInitScriptDisplay, ComponentsCssDisplay {
  /** 是否运行平台内置展示层 init（productSingleUi.client.ts） */
...
/**
 * 解析 productsingle metadata 中的客户端 init 配置。
 *
 * - 未配置 init_script_url / init_script：默认 usePlatformUi=true（平台图集、Tab 等）
 * - 已配置用户 init：默认 usePlatformUi=false；可用 use_platform_ui:true 强制同时启用平台 init
 */
export function resolveProductSingleClientInit(
  metadata: Record<string, unknown> | undefined,
  locale: string
): ProductSingleClientInitConfig {
  const meta = metadata || {};
