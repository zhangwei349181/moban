/**
 * PostSingle 展示层 init 配置（init_script_url / css_url，与 html_url 对称）
 */
...
export interface PostSingleClientInitConfig extends ComponentsInitScriptDisplay, ComponentsCssDisplay {
  /** 是否运行平台内置展示层 init（postSingleUi.client.ts） */
...
/**
 * 解析 postsingle metadata 中的客户端 init / css 配置。
 *
 ...
 */
export function resolvePostSingleClientInit(
  metadata: Record<string, unknown> | undefined,
  locale: string
): PostSingleClientInitConfig {
  const meta = metadata || {};
