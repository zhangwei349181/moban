/**
 * PostList 展示层 init 配置（init_script_url / css_url，与 html_url 对称）
 */
...
export interface PostListClientInitConfig extends ComponentsInitScriptDisplay, ComponentsCssDisplay {
  /** 是否运行平台内置展示层 init（postListUi.client.ts） */
...
/**
 * 解析 postlist metadata 中的客户端 init / css 配置。
 *
 ...
 */
export function resolvePostListClientInit(
  metadata: Record<string, unknown> | undefined,
  locale: string
): PostListClientInitConfig {
  const meta = metadata || {};
